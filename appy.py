...
# --- 4. NAVIGATION (Toujours visible en haut) ---
...
with c2:
    if st.button("COMMUNAUTÉ", use_container_width=True): st.session_state.page = "COMMU"
with c3:
    if st.button("TOP JEUX", use_container_width=True): st.session_state.page = "TOP"
with c4:
...
# --- SI PAGE = ACCUEIL (Ouverture immédiate) ---
...
# --- SI PAGE = COMMUNAUTÉ (S'ouvre SEULEMENT communauté) ---
elif st.session_state.page == "COMMU":
...
# --- SI PAGE = ADMIN (S'ouvre seulement si code, sinon vide) ---
    ...
    if pwd == "628316":
        ...
        # Ici on affiche les stats et les réponses
        st.write(f"Stats : {len(st.session_state.comments)} messages.")
        for i, c in enumerate(st.session_state.comments[::-1]):
            ...
            if not c.get('reply'):
                with st.expander(f"Répondre à {c['user']}"):
                    r = st.text_input("Message", key=f"r_{idx}")
